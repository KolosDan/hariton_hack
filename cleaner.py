...
def big_clean(corpus):
    keyword_mapping = []
    for index, block in enumerate(corpus):
        kwrds = []
        for i in block['ru']:
            kwrds.extend([lemmatizer.lemmatize(w) for w in word_tokenize(i['title'].lower()) if w not in stopwords.words("russian") and w not in string.punctuation + '«»``\'\'' and w not in string.digits and len(w) >= 2])
        c = Counter(kwrds)
        keyword_mapping.append((index, [i[0] for i in c.most_common(20)]))
